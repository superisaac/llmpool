...
use super::{
    ModelResponse, PaginatedResponse,
    bool_mark, print_models, print_pagination,
};
...
pub async fn handle_model(action: ModelAction, client: &ApiClient, json_output: bool) -> Result<(), String> {
    match action {
        ModelAction::List => {
            if json_output {
        ...
        ModelAction::Update {
            ...
            if json_output {
                let raw = client.put_raw(&format!("/models/{}", model_id), &body).await?;
                println!("{}", raw);
            } else {
