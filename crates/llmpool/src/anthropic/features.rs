...
/// Detect whether the upstream supports the Anthropic `/v1/messages` API and
/// update `has_messages` for all models belonging to that upstream.
pub async fn detect_and_save_features(
    pool: &DbPool,
    api_key: &str,
    api_base: &str,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    // 1. Check messages API support once for the whole upstream
    let has_messages = check_messages_api_support(api_key, api_base).await;

    // 2. Find the upstream by api_base
    let upstream = db::llm::get_upstream_by_api_base(pool, api_base).await?;

    // 3. Update has_messages for all models of this upstream
    let models = db::llm::list_models_by_upstream(pool, upstream.id).await?;
    for model in models {
        let update = UpdateLLMModel {
            model_id: None,
            is_active: None,
        ...
        db::llm::update_model(pool, model.id, &update).await?;
    }
