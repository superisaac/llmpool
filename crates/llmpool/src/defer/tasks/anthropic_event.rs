...
use crate::defer::{AnthropicEventData, AnthropicEventTask, BalanceChangeTask};
use crate::models::{
...
/// Represents extracted usage information from an Anthropic response
struct UsageInfo {
    input_tokens: i64,
    output_tokens: i64,
    total_tokens: i64,
...
/// Extract usage information from an AnthropicEventData if available
fn extract_usage(data: &AnthropicEventData) -> Option<UsageInfo> {
    match data {
        AnthropicEventData::CreateMessageResponse(msg) => Some(UsageInfo {
            input_tokens: msg.usage.input_tokens as i64,
            output_tokens: msg.usage.output_tokens as i64,
            total_tokens: (msg.usage.input_tokens + msg.usage.output_tokens) as i64,
            is_batch: false,
        }),
        AnthropicEventData::CreateMessageStreamResponseDone {
            input_tokens,
            output_tokens,
        } => Some(UsageInfo {
        ...
        AnthropicEventData::CreateMessageBatchResponse(batch) => {
            // Batch responses don't carry per-request usage at the batch level;
            // record zero tokens — individual usage is tracked per result item.
            let _ = batch;
            None
        }
        // Request types and legacy completion responses don't carry usage
        _ => None,
    }
}

/// Handle an Anthropic event entry from the async task queue.
///
...
pub async fn handle_anthropic_event(
    event: AnthropicEventTask,
    pool: Data<DbPool>,
    ...
    info!(
        ...
        model_id = model_id,
        "Processing deferred anthropic event"
    );
    ...
    let session_event =
        match db::session_event::create_session_event_with_tx(&mut tx, &new_event).await {
            Ok(ev) => ev,
            Err(e) => {
                warn!(
                    error = %e,
                    session_id = %session_id,
                    "Failed to create anthropic session event"
                );
                return;
            }
        };
    ...
    if let (Some(usage), Some(model)) = (usage, model) {
        ...
        let unique_request_id = format!(
            "anthropic-spendtoken-{}-{}",
            session_event.session_id, session_event.session_index
        );
        let new_change =
            match NewBalanceChange::from_content(account_id, unique_request_id, &content) {
                ...
                Err(e) => {
                    warn!(
                        error = %e,
                        session_id = %session_id,
                        "Failed to serialize anthropic balance change content"
                    );
                    return;
                }
            };
            ...
            match db::session_event::create_balance_change_with_tx(&mut tx, &new_change).await {
                ...
                Err(e) => {
                    warn!(
                        error = %e,
                        session_id = %session_id,
                        "Failed to create anthropic balance change"
                    );
                    return;
                }
            };
        ...
        if let Err(e) = tx.commit().await {
            warn!(
                error = %e,
                session_id = %session_id,
                "Failed to commit anthropic session event transaction"
            );
            return;
        }
        ...
        if let Err(e) = storage.push(entry).await {
            warn!(
                ...
                "Failed to enqueue anthropic balance change task"
            );
        } else {
            info!(
                session_id = %session_id,
                balance_change_id = balance_change.id,
                "Enqueued anthropic balance change task"
            );
        }
    ...
    match tx.commit().await {
        Ok(()) => {
            info!(
                session_id = %session_id,
                "Successfully processed deferred anthropic event"
            );
        }
        Err(e) => {
            warn!(
