...
/// Middleware that authenticates requests using the `x-api-key` header (Anthropic API style).
/// Functionally identical to `auth_openai_api` but reads the token from `x-api-key` instead of
/// the `Authorization: Bearer <token>` header.
pub async fn auth_anthropic_api(
    State(state): State<Arc<AppState>>,
    request: Request,
    next: Next,
) -> Response {
